...
def simulation(m_d, m_b, m_c, tmax, particle_indices, core_id, a_group):
    ...
    filename = f'core_outputs_yr2/core_{core_id}_{tmax}_yr_{N_particles}_3_pl_w_captures_and_saving_every_100_yrs_TEST.nc'
    ...
    with netCDF4.Dataset(filename, 'w') as ncfile:
        ...
        #ncfile.description = f'simulation results from core {core_id}. Trevascus 2025 values (inc masses).3 planets!! "peri mig" when a(1-e)<18. do not delete particles when migrated. tracking captures and collisions. saving every 100 yrs.'
        ncfile.description = 'this is a test'
        ...
        for t, tid in enumerate(times):
            ...
            if not (c1 and c2):
                with open(f'progress_tracking_files/failed_cores_3pl_outerdisc.txt', 'a') as f:
            ...
            for p in sim.particles[sim.N_active:]:
                ...
                if (p.a*(1-p.e)) < 18 and p.e < 1 and not traj['migrated_peri']:
                    ...
                    for j, plhash in enumerate(['star', 'pb', 'pc', 'pd']):
                        pl = sim.particles[plhash]
                        if plhash != 'star':
                            massive_bods_var[count_migrated_peri+1, j, :] = [tid, pl.x, pl.y, pl.z, pl.e, pl.a, pl.inc, pl.f, pl.Omega, pl.omega, 22222]
                        else:
                            massive_bods_var[count_migrated_peri+1, j, :] = [tid, pl.x, pl.y, pl.z, 0, 0, 0, 0, 0, 0, 11111]
                    count_migrated_peri += 1

                # if p.a < 18 and p.e < 1 and not traj['migrated']:
                #     traj['migrated'] = True
                ...
                #         pl = sim.particles[plhash]
                #         if plhash != 'star':
                #             massive_bods_var[count_migrated_peri+count_migrated, j, :] = [tid, pl.x, pl.y, pl.z, pl.e, pl.a, pl.inc, pl.f, 22222]
                #         else:
                #             massive_bods_var[count_migrated_peri+count_migrated, j, :] = [tid, pl.x, pl.y, pl.z, 0, 0, 0, 0, 11111]

                #     count_migrated += 1
                    #to_remove.append(hash.value)

                #mig = traj['migrated'] or traj['migrated_peri']
        ...
        #this bit is wrong in capy_files ...2_pl.nc, p = sim.particles[i+4] instead so it skips a particle essentially
        # for i in range(len(sim.particles)-sim.N_active):
        #     p = sim.particles[i+sim.N_active]
        #     test_particles_var[1, i, :] = [p.x, p.y, p.z, p.e, p.a, p.inc, p.f, p.Omega, p.omega, p.hash.value]
    ...
    with open(f'progress_tracking_files/progress_{tmax}_yr_3pl_outerdisc.txt', 'a') as f:
...
#%%
def prompt():
    confirmation = input('Ally, check what script you are running. Then, did you remember to change the name of your file and update file description? Type "YES" if so.')
...
def parallelization(N_testparticles, tmax, N_cores):
    ...
    with open(f'progress_tracking_files/progress_{tmax}_yr_3pl_outerdisc.txt', 'w') as f:
        pass
    ...
    chunk_results = Parallel(n_jobs=N_cores)(
        delayed(simulation)(m_d, m_b, m_c, tmax, group, core_id, a_group) for core_id, (group, a_group) in enumerate(zip(groups, a_groups))
    )
