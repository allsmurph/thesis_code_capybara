...
def simulation(m_d, m_b, m_c, tmax, particle_indices, core_id, a_group):
    ...
    trajectories = {create_unique_hash(i, core_id) : {'ejected': False, 'migrated': False, 'migrated_peri': False, 'collided': False} for i in particle_indices}
    ...
    with netCDF4.Dataset(filename, 'w') as ncfile:
        ...
        ncfile.description = f'simulation results from core {core_id}. Trevascus 2025 values (inc masses).2 planets!! "mig" when a<18 au and "peri mig" when a(1-e)<18. do not delete particles when migrated. deleting collided particles'
        ...
        for t, tid in enumerate(times):
            ...
            c1 = np.all(eccs < 1)
            c2 = (apo_dist[0] < peri_dist[1] ) #and (apo_dist[2] < peri_dist[0])
            ...
            if t == 0:
                for j, hash in enumerate(['star', 'pb', 'pc']):
                    ...
                    if hash != 'star':
                        massive_bods_var[0, j, :] = [t, p.x, p.y, p.z, p.e, p.a, p.inc, p.f, 22222]
                    else:
                        massive_bods_var[0, j, :] = [t, p.x, p.y, p.z, 0, 0, 0, 0, 11111]

            elif tid == tmax:
                for j, hash in enumerate(['star', 'pb', 'pc']):
                    p = sim.particles[hash]
                    if hash != 'star':
                        massive_bods_var[count_migrated_peri+count_migrated+1, j, :] = [tid, p.x, p.y, p.z, p.e, p.a, p.inc, p.f, 22222]
                    else:
                        massive_bods_var[count_migrated_peri+count_migrated+1, j, :] = [tid, p.x, p.y, p.z, 0, 0, 0, 0, 11111]
